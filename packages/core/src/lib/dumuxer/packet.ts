...
export function demux_packet(
  ...
  if (mem.getUint8(ptr) !== SYNC_BYTE) {
    return 2;
  } // invalid packet sync byte
  ...
  if (flags & 0x20) {
    ...
    if (adaptationFieldLength + 1 > len) {
      return 5;
    } // Adaptation Field Overflows File Length
    ...
    if (adaptationFieldLength > 0) {
      ...
      if (hasPCR) {
        const pcrBase =
          (mem.getUint32(ptr + 2) << 1) | (mem.getUint8(ptr + 6) >> 7);
