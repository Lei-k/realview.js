...
export function decode_pes(
  ...
  start: if (pstart) {
    ...
    if (len < hlen) {
      return 24;
    } // PES Header Overflows File Length
    if (l > 0) {
      l -= hlen;
    }
    ...
    switch (bitmap & 0xc0) {
      case 0x80: {
        ...
        if (s.has_dts && pts !== s.dts) {
          s.frame_ticks = pts - s.dts;
        }
        if (pts > s.last_pts || !s.has_pts) {
          s.last_pts = pts;
        }

        if (
          s.first_pts === 0 &&
          s.frame_num === (s.content_type === content_types.video ? 1 : 0)
        ) {
          s.first_pts = pts;
          s.first_pcr = s.pcr ?? 0;
        }
      ...
      case 0xc0: {
        ...
        if (s.has_dts && dts > s.dts) {
          s.frame_ticks = dts - s.dts;
        }
        if (pts > s.last_pts || !s.has_pts) {
          s.last_pts = pts;
        }
