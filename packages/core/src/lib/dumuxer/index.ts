...
export type { Packet, Stream, StreamTypes, ContentTypes, PACKET_LEN, ErrCodes };
...
export class TSDemuxer {
  ...
  process(
    ...
    for (let ptr = offset; ; ptr += PACKET_LEN) {
      ...
      if (datalen === 0) {
        return 0;
      } // ended on a complete packet
      if (datalen < PACKET_LEN) {
        // insufficient data for another complete packet
